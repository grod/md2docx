    ...
    async (node) => {
      try {
        ...
        if (node.url.startsWith("data:")) {
          ...
          if (enc !== "base64") {
            log.warn(
              `[${idx}] Error decoding data url. unknown encoding: ${enc}`,
            );
            return;
          }
          ...
          if (node.url.startsWith("res:")) {
            if (!resourceLoader) {
              log.warn(
                `[${idx}] Error loading image ${node.url}. resource loader missing.`,
              );
              return;
            }
          ...
          log.info(
            `[${idx}] ${ret.status} ${hsize(buffer.length).padStart(10)} ${type}`,
          );
        }
